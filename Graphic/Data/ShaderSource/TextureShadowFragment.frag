...
vec3 CalcLight(Light light, vec3 norm, vec3 viewDir, int index, vec3 Texture)
{
	...
	for(int i = 0; i < 20; ++i)
	{
		float closestDepth = texture(depthCubeMap[index], fragToLight + sampleOffsetDirections[i] * diskRadius).r;
		closestDepth *= 100.0;
...
vec3 CalcFlashlight(Flashlight flash, vec3 norm, vec3 viewDir, vec3 Texture)
{
	...
	vec3 diffusecolor = vec3(1,1,1) * 2.0 * diff * Texture;
	
	vec3 reflectDir = reflect(-lightDir, norm);
	float Spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess * 128.0);
	vec3 specularcolor = vec3(1,1,1) * Spec * texture(material.texture2, TexCoord).rgb;;
	//vec3 specularcolor = light.specular * Spec * texture(material.texture2, TexCoord).rgb;;
...
void main()
{
	...
	{
		result += CalcLight(light[i], norm, viewDir, i, Texture.rgb);
		//result = clamp(result, 0.0, 0.1);
	}
	if(isflash)
	{
